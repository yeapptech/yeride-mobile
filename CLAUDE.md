...
**Last updated:** May 4, 2026 (Phase 9 Turn 15 closed — NavigationSdk teardown telemetry flips: closes the deferred follow-up Turn 12's close logged, completing the Crashlytics rawMeta-channel saturation across the file. Three `LOG.warn → LOG.error` flips at L387/L415/L428 in `src/data/services/NavigationSdkClient.ts` (post-flip line numbers L405/L442/L466). All three are cleanup-best-effort teardown swallows that prior Turn 4 / Turn 11 / Turn 12 audits classified as stays-warn for legitimate reasons (failures don't propagate, next session's init recovers cleanly). Turn 15 reverses that decision now (rather than waiting for post-cutover field signals) on three rationales: (a) Crashlytics is graceful about non-actionable noise — sample rates are observable and demoting back to warn is a single-line revert per site; (b) two of the three sites (L415 / L428) currently have ZERO telemetry surface — failures are intentionally swallowed inside `cleanup()` and never propagate up the Result channel, so flipping is the only way to ever see them; (c) the standalone L387 path's `Result.err(NetworkError)` is visible to the navigation VM via `onEndNavigation`'s fire-and-forget call (which discards the Result) and the unmount-cleanup chain (which logs at `LOG.warn`, not fanning out) — flipping the SDK-side breadcrumb to error captures both call paths in one place. All four pre-checklist questions landed on the Recommended option, so the full surface ships in one turn. Adapter↔VM duplicate-noise tradeoff confirmed N/A today: the VM-side teardown sites at `useDriverNavigationViewModel.ts` L296/L300 are at `LOG.warn`, which does NOT fan out through the rawMeta channel to `recordError` (Phase 9 Turn 6 contract — only `LOG.error` does). Flipping the SDK adapter sites produces zero duplicate Crashlytics records today; the Turn 9 framing doesn't apply unless the VM warns are also flipped (deferred). Constructed-Error wrappers NOT needed: all three sites have `e` from a synchronously-thrown SDK call (rejected Promise from `stopGuidance` / `setOnArrival`), so `e instanceof Error` is already true and reference identity flows through rawMeta directly — mirrors Turn 12's L520 verbatim. Each flip carries an inline JSDoc comment block explaining the level + Error-shape choice (failure NOT propagated up the call stack; reference identity preserved; documents the L387↔VM `useDriverNavigationViewModel` non-duplicate today). +3 new regression tests in `NavigationSdkClient.test.ts` under a new describe block `'telemetry — recordError fan-out via rawMeta channel (Phase 9 turn 15)'`: each test attaches a `CrashlyticsLogTransport(fakeCrash)` to the singleton `LOG`, drives the failure path via the existing test mocks (`controller.stopGuidance.mockRejectedValueOnce` for L387/L466; `listeners.setOnArrival.mockImplementation(arg => { if (arg === null) throw }` for L442 — the listener is called once on subscribe with the handler then again with null on cleanup, so we throw only on the null arg), and asserts `getRecordedErrors()` contains a record with (a) reference identity on the seeded `Error` (proves rawMeta channel preserved through `sanitizeForLogging`), (b) `name === 'YeRide:NavigationSdk'` so Firebase Console groups under the correct scope, (c) message substring matching the inline log message (catches future cosmetic edits to the grouping key). `try/finally { LOG.removeTransport(transport) }` per Turn 4 / 8 / 9 / 11 / 12 / 14 hygiene. Standalone-path test additionally pins `result.error.code === 'navigation_stop_guidance_failed'` so the LOG-fan-out doesn't replace the wrapped NetworkError surface. Stays-warn audit re-verified: post-Turn-15 `grep -c "logger\.warn" NavigationSdkClient.ts` returns 0; all 9 LOG sites in the file are at error. Acceptance: **187 suites / 1624 tests** (+0 suites / +3 tests over Turn 14's 187/1621, exactly at the floor of the kickoff's "+0 suites / +3 tests" estimate band — one regression test per flipped site, no optional sanity checks). **No native rebuild required** — pure TS refactor; no new dependencies; no plugin patches; no DI container changes; no cross-repo work. Rollback is one `git revert` deep — flipped lines are uniform comment + single-token edits co-located in one file; tests live under one new describe block. Manual smoke is user-driven: pure telemetry; field-validation note documented in `docs/PHASE_9_TURN_15.md` § "Smoke checklist (user-driven)" — after the next deploy lands, watch Firebase Console for the new per-teardown-site issues to populate; sustained zero-rate on a flipped site flags either dead telemetry (revisit warn-stay decision) or genuinely robust SDK-state handling, sustained non-zero rate flags a real stale-controller race between React Navigation's screen unmount and the SDK's internal lifecycle worth investigating, high-volume firing (>1% of trip-end flows) → demote back to warn while the underlying bug is investigated separately.)
...
| 9 turn 16+            | Future Phase 9 polish (receipt PDF) or roll into Phase 10 cutover prep                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Pending                        |
