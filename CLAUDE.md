...
in the Critical files table). A second out-of-band fix landed on
...
`animateToRegion`. See
`docs/PHASE_10_OOB_DRIVER_HOME_STALE_LOCATION.md`. The dev-mode
short-circuit on BG-geolocation means foreground reads are the only
GPS source on the home screens in `__DEV__`, which is why the
fallback chain matters there. See `docs/PHASE_*.md` for the
...
| `src/data/services/BackgroundGeolocationClient.ts`                          | 11-method GPS SDK seam; listener-deduped; v5 compound config; carries a `__DEV__` short-circuit on every native method to dodge an upstream `tslocationmanager:4.1.5` priority-translation crash on the emulator (production builds run native paths unchanged)                             |
...
| `src/presentation/hooks/useCurrentLocation.ts`                              | Foreground location read for `*HomeScreen` + `RouteSelect`. Three-tier cold-start fallback (fresh capped last-known → live `getCurrentPositionAsync` → uncapped last-known); guards against the dev-build "stale last-dropoff" symptom while BG-geolocation is short-circuited in `__DEV__` |
