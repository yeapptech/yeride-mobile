...
- **§4 withMaterialTheme** flipped ⚠️ → ❌. Stripe `CardForm` (used
  in `AddPaymentMethodScreen`) requires Material Components theme
...
| `withMaterialTheme` custom plugin               | Present                                                               | ❌ not found                                                                                             | ❌ **needed** (verified Turn 1). Stripe `CardForm` in `AddPaymentMethodScreen.tsx` requires Material Components Android theme; `@stripe/stripe-react-native@0.63.0`'s own plugin only handles Apple Pay entitlement / Onramp / Google Pay meta-data — no Material theme. Without porting, Android CardForm render crashes.                                                                                                                                                                                                                                                                                                                                                          |
